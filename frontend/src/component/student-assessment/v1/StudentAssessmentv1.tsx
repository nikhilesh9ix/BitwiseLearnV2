...
} from "@/api/assessments/get-assessments-by-batch";

const Colors = getColors();

type StudentAssessment = AssessmentListItem & {
  instructions: string;
...
const normalizeAssessment = (
  ...
  ...assessment,
  instructions: assessment.instructions ?? assessment.instruction ?? "",
  canAccessTest: assessment.canAccessTest ?? false,
});
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span>
          {new Date(assessment.startTime).toLocaleString()} to{" "}
  ...
  useEffect(() => {
    const fetchAssessments = async () => {
      try {
        ...
        if (studentBatchId) {
          const batchAssessments = await getAssessmentsByBatch(
            undefined,
            studentBatchId,
          );
          setAssessments(batchAssessments.map(normalizeAssessment));
          return;
        }

        const dashboardRes = await axiosInstance.get("/api/v1/students/dashboard");
        const rawAssessments = Array.isArray(dashboardRes.data?.data?.assessments)
          ? (dashboardRes.data.data.assessments as AssessmentListItem[])
          : [];

        setAssessments(rawAssessments.map(normalizeAssessment));
      } catch {
        setAssessments([]);
      } finally {
        setLoading(false);
