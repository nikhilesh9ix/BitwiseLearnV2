...
function DashboardInfo({ data }: Props) {
  ...
  return (
    <>
      {/* Table Container */}
      <div
        className={`w-full overflow-y-auto overflow-x-hidden ${Colors.border.defaultThick} ${Colors.background.secondary} shadow-lg`}
        style={{
          scrollbarWidth: "none",
        ...
        <table className="w-full border-collapse">
          ...
          <tbody className="divide-y divide-white/5">
            {data.map((item) => (
              <tr
                key={item.id}
                className={`text-sm ${Colors.text.primary} transition-colors`}
              >
